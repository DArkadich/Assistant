...
Система семантического поиска документов с использованием векторных эмбеддингов.
"""
...
class RAGSystem:
    ...
    # Методы для совместимости с ботом
    def search(self, query, limit=5):
        """Алиас для search_documents для совместимости."""
        return self.search_documents(query, limit)
    
    def get_all_documents(self):
        """Получить все документы (для совместимости)."""
        if not self.collection:
            return []
        ...
        try:
            ...
            print(f"❌ Ошибка получения всех документов: {e}")
            return []
